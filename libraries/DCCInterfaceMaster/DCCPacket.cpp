...
DCCPacket::DCCPacket(uint16_t new_address /*, uint8_t new_address_kind*/) : address(new_address), address_kind(/*new_*/address_kind), kind(idle_packet_kind), size_repeat(0x40) //size(1), repeat(0)
{
  address_kind = DCC_SHORT_ADDRESS;		//short or long Address needs to use
  if (new_address > MAX_DCC_SHORT_ADDRESS)					//use long Address if the Address is to high!
...
uint8_t DCCPacket::getBitstream(uint8_t rawbytes[]) //returns size of array.
{
	...
	if (kind & MULTIFUNCTION_PACKET_KIND_MASK) {
		...
		else if ((address_kind == DCC_LONG_ADDRESS) && (kind != ops_mode_programming_kind)) //This is a 14-bit address
		{
