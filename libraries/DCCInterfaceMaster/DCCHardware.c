...
uint8_t DCCPin = 6;
uint8_t DCCPin2 = 0;	//for RailCom support
...
#endif
uint8_t oldstate = LOW;	//state of the output
uint8_t RailComActiv = false;
...
*/
  ...
  if (RailComActiv == false) {
	/*  
	  #if defined(DCC_USE_TRUE_C)
	  DCC_PIN_TOGGLE();
	  #else
	*/	  
	  digitalWrite(DCCPin, oldstate);
	//  #endif
	#if defined(RAILCOM)
	  digitalWrite(DCCPin2, oldstate);
	#endif  
  }  
  ...
  if (oldstate == LOW)
  {
	  ...
	  else DCC_TMR_OUTP_ONE_COUNT();
#endif
#if defined(RAILCOM)
	if (DCC_state == dos_send_preamble && current_bit_counter == (PREAMBLE_LENGTH - 4)) {
		RailComActiv = false;
		digitalWrite(DCCPin, oldstate);
		digitalWrite(DCCPin2, oldstate);
	}
    ...
    switch(DCC_state)
    {
      ...
      case dos_idle:
        ...
        {
			DCC_TMR_OUTP_ONE_COUNT(); //just send ones if we don't know what else to do. safe bet.
//          Serial.println("X");
          break;
        }
		/*
		...
		*/
		...
		#endif
      /// Preamble: In the process of producing 16 '1's, counter by current_bit_counter; when complete, move to dos_send_bstart
