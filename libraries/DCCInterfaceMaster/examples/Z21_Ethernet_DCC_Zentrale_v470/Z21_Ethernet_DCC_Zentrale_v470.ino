...
Aug.2016   add Railcom Support and update DCCInterfaceMaster, 
...
/**************************************************************
DCC internal Booster with hardware modification for Railcom*/
//#define RAILCOM    //activate RailCom support (over GO-Internal-Pin!)
...
static void globalPower (byte state) {
  ...
  switch (state) {
    case csNormal: 
      ...
      #if (defined(BOOSTER_INT) && !defined(RAILCOM))
    ...
    case csShortCircuit: 
      #if defined(DCC)
      dcc.setpower(OFF);  //shut down via GO/STOP just for the Roco Booster
      #endif
      #if defined(BOOSTER_EXT)
      digitalWrite(GoExtPin, BOOSTER_EXT_OFF);
      #endif
...
{     
  ...
  else if (XNetReturnLocoFkt == true) {
