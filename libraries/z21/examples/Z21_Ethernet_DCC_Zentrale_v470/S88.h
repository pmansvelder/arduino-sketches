...
#elif defined(SANGUINO_MCU) //Sanguino (other pins!)
#define S88useT3  //S88 mit Timer3
...
void SetupS88() {
  ...
  if (S88Module > S88MAXMODULE) { //S88 on!
    ...
    EEPROM.update(EES88Moduls, S88MAXMODULE); //save correct max Value
  }
...
//--------------------------------------------------------------------------------------------
void reportS88Data(byte group) {
  byte MAdr = 1;  //Rückmeldegruppenmodul
  byte datasend[11];  //Array Gruppenindex (1 Byte) & Rückmelder-Status (10 Byte)
  datasend[0] = group; //request Gruppenindex

  for(byte m = (group * 10); m < S88Module; m++) {  //Durchlaufe alle aktiven Module im Speicher
