...
unsigned int S88RMCount = 0;   //Lesezähler Modul-Pin (S88Module * 8)
...
void SetupS88() {
  ...
  S88Module = 255; //EEPROM.read(EES88Moduls);
...
void getS88Data() {       //kein Timer
  ...
  else if (S88RCount >= 12) { //S88RCount < 10 + (S88Module * 8) * 2) {    //Auslesen mit weiteren Schiebetakt der Latches links
  ...
  if (S88RMCount >= (S88Module * 8)) {  //Alle Module ausgelesen?
