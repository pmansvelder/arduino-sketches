/*
          <========Arduino Sketch for Arduino Due with Ethernet shield W5100=========>
          Locatie: Zolder
          ...
          Arduino Due with W5100 Ethernetshield used as MQTT client
          It will connect over Wifi to the MQTT broker and controls digital outputs (LED, relays)
          The topics have the format "domus/zolder/uit" for outgoing messages and
...
const char* topic_out = "domus/zolder/uit";               // Relaisuitgangen: R<relaisnummer><status>
const char* topic_out_smoke = "domus/zolder/uit/rook";    // MQ-2 gas & rookmelder, geconverteerd naar 0-100%
const char* topic_out_light = "domus/zolder/uit/licht";   // LDR-melder: 0=licht, 1=donker
const char* topic_out_pulse = "domus/zolder/uit/deur";    // Pulserelais t.b.v. deuropener

//const char* topic_out_temp = "domus/zolder/uit/temp";     // DHT-22 temperatuursensor
//const char* topic_out_hum = "domus/zolder/uit/vocht";     // DHT-22 luchtvochtigheid
//const char* topic_out_heat = "domus/zolder/uit/warmte";   // DHT-22 gevoelstemperatuur

const char* topic_out_temp = "domus/zolder/uit/temp";           // BMP280 temperatuur
const char* topic_out_pressure = "domus/zolder/uit/pressure";   // BMP280 Luchtdruk
const char* topic_out_altitude = "domus/zolder/uit/altitude";   // BMP280 hoogte

const char* topic_out_pir = "domus/zolder/uit/pir";       // PIR sensors
...
void setup() {
  ...
  if (debug) {
    ...
    ShowDebug(F("MQTT Arduino Domus Zolder"));
    ShowDebug(hostname);
    ShowDebug("");
  }
...
void reconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(CLIENT_ID)) {
      ...
      String messageString = ip + ":" + CLIENT_ID + " ready.";
      messageString.toCharArray(messageBuffer, messageString.length() + 1);
      mqttClient.publish(topic_out, messageBuffer);
      // ... and resubscribe
...
void sendData() {
  ...
  messageString.toCharArray(messageBuffer, messageString.length() + 1);
  mqttClient.publish(topic_out_pressure, messageBuffer);
...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  //
    ...
    String messageString = "Reset command received, resetting in one second...";
    messageString.toCharArray(messageBuffer, messageString.length() + 1);
    mqttClient.publish(topic_out, messageBuffer);
    delay(1000);
    resetFunc();
