...
void report_saved_data() {
  ShowDebug("EEPROM data");
  ShowDebug("Saved     : " + String(configuration.saved));
  ShowDebug("State     : " + String(configuration.state));
  ShowDebug("Brightness: " + String(configuration.brightness));
}
...
void callback(char* topic, byte * payload, byte length) {
  ...
  if (String(topic).indexOf(cmd_topic_lights) >= 0) {
    ...
    if (LightBrightness[index - 1] && doc.containsKey("brightness")) {
      ...
      configuration.state = true;
      configuration.brightness = doc["brightness"];
      configuration.saved = true;
      EEPROM_writeAnything(0, configuration);
      EEPROM.commit();
      ShowDebug("write EEPROM data");
      report_saved_data();
      EEPROM_readAnything(0, configuration);
      ShowDebug("read EEPROM data");
      report_saved_data();
    }
    else {
