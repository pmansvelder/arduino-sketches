/*
    ...
    Neu:
    - Serail Debug Funktion für Arduino MEGA
    - Enc28j60 kompartibel (keine sichere Kommunikation!!!)
    - S88 Rückmelde Bus
 */

// hashmaps for storage of loc type

#include <HashMap.h>
const byte HASH_SIZE = 64;
// storage
HashType<int, int> hashRawArray[HASH_SIZE];
HashMap<int, int> LocTypeMap = HashMap<int,int>( hashRawArray, HASH_SIZE );
...
byte mac[6] = { 0x84, 0x2B, 0xBC, 0xEF, 0xFE, 0xED };
...
void Ethreceive() {
  ...
  if(packetSize > 0) {
    ...
    case 0x30:
      ...
      case (0x60):
        #if SerialDEBUG 
          Debug.print("LAN_GET_LOCOMODE: ");
          address = word(packetBuffer[4] & 0x3F, packetBuffer[5]);
          Debug.println(address);
          Debug.print("Index: ");
          Debug.print((LocTypeMap.getIndexOf(address), DEC));
          Debug.print(", Value: ");
          Debug.println(LocTypeMap.getValueOf(address));
          data[0] = packetBuffer[4];
          data[1] = packetBuffer[5];
          data[2] = LocTypeMap.getValueOf(address);
          EthSend (0x07, 0x60, data, false, 0x00); 
        #endif
      break;
      case (0x61):
        address = word(packetBuffer[4] & 0x3F, packetBuffer[5]);
        #if SerialDEBUG 
          Debug.print("LAN_SET_LOCOMODE: "); 
          Debug.println(address);
        #endif
        LocTypeMap[locindex](address,int(packetBuffer[6]));
        locindex++;
        Debug.print("Value: ");
        Debug.print(int(packetBuffer[6]));
        Debug.print(" : ");
        Debug.println(LocTypeMap.getValueOf(address));
      break;
      case (0x70):
...
void notifyLokAll(uint8_t Adr_High, uint8_t Adr_Low, boolean Busy, uint8_t Steps, uint8_t Speed, uint8_t Direction, uint8_t F0, uint8_t F1, uint8_t F2, uint8_t F3, boolean Req ) {
  byte DB2 = Steps;
  address = word(Adr_High & 0x3F, Adr_Low);
  if (LocTypeMap.getValueOf(address) == 1)
    DB2 = 2;
